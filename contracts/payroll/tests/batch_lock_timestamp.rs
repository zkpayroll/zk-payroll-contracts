//! Batch lock timestamp query helper tests (#401).
...
#[test]
fn test_batch_lock_timestamp_non_existent() {
    let env = Env::default();
    let (payroll, _admin, _treasury, _treasury_owner, _employee) = setup_payroll(&env);

    assert_eq!(payroll.get_batch_lock_timestamp(&123), None);
}

#[test]
fn test_batch_lock_timestamp_lifecycle() {
    let env = Env::default();
